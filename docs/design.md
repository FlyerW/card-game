...
> 狀態：v0.8 規則草案｜規則引擎已實作（`packages/engine`，全部 164 個測試）｜網頁版可以跟電腦試玩、自己組牌，也能兩人連線（`packages/web`、`packages/server`）｜最後更新：2026-09-26
...
- 英雄對戰的模擬裡，五個英雄對其他英雄的勝率從 31.8%–75.8% 拉近到 46.6%–54.0%（見[平衡模擬](#平衡模擬)）；拿掉即時回應後先攻稍微有利一點（51%–53%）
...
| 能量制度 | 先攻勝率（三種打法） | 判讀 |
|---|---|---|
| 舊制：從 1 開始每回合 +1、最高 10、後攻第一回合補 1 點 | 62.1% – 64.6% | 先攻明顯有利 |
| **新制：先攻 1、後攻 2，每回合 +2、最高 12** | **51.3% – 52.6%** | **差距很小** |
| 新制但後攻不補償：雙方都從 1 開始 | 68.7% – 71.1% | 先攻明顯有利 |

| 英雄 HP（新制、均衡） | 25 | 30 | **35** | 40 | 50 |
|---|---|---|---|---|---|
| 先攻勝率 | 49.8% | 51.3% | **51.8%** | 52.0% | 51.9% |
| 平均回合數（雙方合計） | 16.7 | 18.2 | **19.6** | 20.9 | 23.1 |
| 預估時間（中位數） | 5.7 分 | 6.2 分 | **6.7 分** | 7.1 分 | 8.0 分 |
| 落在 5–10 分鐘的比例 | 62.7% | 71.3% | **77.3%** | 78.8% | 75.5% |

| 英雄 | HP | 鏡像先攻勝率 | 預估時間（中位數） | 對其他英雄的平均勝率 |
|---|---|---|---|---|
| 烈焰領主（紅） | 32 | 50.8% | 5.1 分 | 54.0% |
| 林海之王（綠） | 33 | 48.8% | 7.7 分 | 50.5% |
| 無名劍士（白） | 35 | 52.4% | 7.7 分 | 49.5% |
| 潮影雙生（藍黑） | 32 | 48.9% | 7.8 分 | 49.5% |
| 虹彩賢者（五色） | 29 | 48.4% | 7.1 分 | 46.6% |

**結論**

1. **新制還是平衡的，拿掉即時回應後先攻稍微有利一點**：三種打法 51.3%–52.6%，只有控場打法的信賴區間
   （50.5%–54.8%）剛好整個在 50% 以上。v0.7 是 49.3%–51.4%。後攻的人少了在對手回合出手的機會，
   差距還很小，先不動；試玩時覺得先攻太有利，再考慮給後攻多一點補償。
   沒有後攻多的那 1 點能量，先攻勝率接近七成。
2. **英雄 HP 不影響先後手平衡，只影響對局長度**：每多 5 HP，平均多 1.3 個回合。
   **35 HP 預估中位數 6.7 分鐘，77% 的對局落在 5–10 分鐘**；40 HP 是 78.8%，差不多。
3. **英雄之間的差距拉近了**：v0.7 是紅色 75.8%、虹彩賢者 31.8%，v0.8 五個英雄都在 46.6%–54.0%。
   最懸殊的一組是烈焰領主對虹彩賢者 62.6%。紅色的對局還是最短（5.1 分），最快的一成不到 4 分鐘。
...
1. **英雄強弱**：v0.8 調過之後，電腦對打裡五個英雄對其他英雄的勝率都在 46.6%–54.0%（見[平衡模擬](#平衡模擬)）。
