...
| 英雄 | 顏色 | HP | 效果 |
|---|---|---|---|
| 無名劍士 | 白 | 72 | 被動「劍士之道」：我方生物 ⚔ +1 |
| 深海先知 | 藍 | 52 | 天生技 預見（能量 3）：抽 1 張 |
| 冥府祭司 | 黑 | 44 | 天生技 蝕心（能量 4）：對手隨機棄 1 張 |
| 烈焰領主 | 紅 | 62 | 天生技 燃燼（能量 1）：任意目標 1 傷害 |
...
| 潮影雙生 | 藍黑 | 38 | 天生技輪流：潮之面（能量 3）抽 1 張，用完變成影之面（能量 4）對手隨機棄 1 張，再變回來 |
...
HP 的起點是 55 − 3 ×（顏色數 − 1）− 效果強度，再照英雄對戰的模擬調。攻擊範圍與種族加進來之後，快攻英雄變弱、
對局變長，抽牌棄牌這類累積優勢的英雄變強，所以快攻英雄的 HP 拉高（無名劍士 72、烈焰領主 62），控場英雄的 HP 壓低
（潮影雙生 38、虹彩賢者 40、冥府祭司 44）。**只調 HP 已經調不平了**，見下面的模擬結果。英雄進化加的 HP 是 9–11。
...
這一版加了攻擊範圍（正前方與兩個斜對角）、種族、自己付的能量上限 −X、潮影雙生輪流的天生技、虹彩賢者的新技能。

| | 加攻擊範圍與種族之後 | 之前 | v0.8 |
|---|---|---|---|
| 新制先攻勝率（三種打法） | 54.3% – 56.5% | 51.5% – 53.4% | 51.6% – 53.0% |
| 基準英雄的預估時間（中位數） | 7.2 分（55 HP） | 6.3 分 | 6.7 分（35 HP） |
| 落在 5–10 分鐘 | **88%** | 88% | 78% |
| 英雄對其他英雄的平均勝率 | **41.0% – 57.1%** | 48.3% – 51.4% | 46.6% – 54.8% |
| 牌庫抽完的對局 | 2.7% | 0.5% | 0.3% |

| 英雄 HP（模擬英雄） | 35 | 45 | **55** | 65 | 75 |
|---|---|---|---|---|---|
| 預估時間（中位數） | 5.7 分 | 6.5 分 | **7.2 分** | 7.9 分 | 8.6 分 |
| 落在 5–10 分鐘 | 71% | 88% | **88%** | 82% | 73% |

| 英雄 | HP | 鏡像先攻勝率 | 平均攻擊次數 | 預估時間（中位數） | 對其他英雄 |
|---|---|---|---|---|---|
| 潮影雙生（UR） | 38 | 52.6% | 21.7 | 8.7 分 | 57.1% |
| 林海之王 | 54 | 55.4% | 24.8 | 8.2 分 | 53.9% |
| 冥府祭司 | 44 | 49.4% | 18.8 | 7.7 分 | 52.3% |
| 翠林聖女（UR） | 58 | 54.7% | 26.8 | 7.3 分 | 51.8% |
| 深海先知 | 52 | 51.2% | 25.0 | 10.0 分 | 48.2% |
| 虹彩賢者（UR） | 40 | 53.3% | 15.6 | 6.3 分 | 48.0% |
| 烈焰領主 | 62 | 56.6% | 13.5 | 7.7 分 | 47.7% |
| 無名劍士 | 72 | 56.4% | 28.0 | 8.1 分 | 41.0% |

1. **攻擊範圍讓對局變長**：擋在前面的生物會保護英雄，打到英雄的次數變少，55 HP 從 6.3 分變成 7.2 分，牌庫抽完的對局也變多（2.7%）。
   一開始做成「三格全空才打得到英雄」時更誇張（8–12 分鐘），所以改成有一格空著就打得到。
2. **快攻變弱、累積優勢變強**：對局一長，抽牌棄牌的英雄（潮影雙生、冥府祭司）佔優勢；靠場面壓人的無名劍士只有 41.0%，
   HP 已經拉到 72 還是最弱。要再調平，得改規則或英雄的效果，不只是 HP（見「待你決定」）。
3. **先攻更有利**：54%–57%。先下場的生物先卡住位置，後攻要打穿比較難。可能需要給後攻補償（例如爐石的幸運幣）。
...
1. **攻擊範圍之後的平衡**（大縮模實驗）：八個英雄 41.0%–57.1%，只調 HP 調不平。可以考慮：
   ...
   - 潮影雙生：影之面已經從 3 費改成 4 費，電腦對打裡只從 57.9% 降到 57.6%，強的地方主要在藍黑的卡池（控場、亡靈、能量上限 −X），
     不在天生技；要再調得看是哪些卡；
   - 給後攻補償，先攻現在 54%–57%。
