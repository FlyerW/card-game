...
> 狀態：v0.7 規則草案｜規則引擎已實作（`packages/engine`，全部 168 個測試）｜網頁版可以跟電腦試玩、自己組牌（`packages/web`）｜最後更新：2026-09-25
...
- **範例卡擴充到 88 張**：每個顏色 12–16 張、無色 8 張，單色英雄組得出正式的 40 張牌組；
...
- **每個顏色 12–16 張、無色 8 張**（英雄進化卡另計）。單色英雄有 20–24 種卡可以用，組得出正式的 40 張牌組
...
| 預估時間（平均） | 5.9 分 | 6.5 分 | **7.1 分** | 7.6 分 | 8.6 分 |
...
2. **英雄 HP 不影響先後手平衡，只影響對局長度**：每多 5 HP，平均多 1–1.5 個回合。
   **35 HP 最符合一局 5–10 分鐘**：預估平均 7.1 分鐘，77% 的對局落在 5–10 分鐘，是幾個 HP 裡最高的。
