...
| **沉默** | 不能發動技能(原本或已發動的效果失效)，攻擊照常 | 同上 | 白 |
| **虛弱** | 不能攻擊/反擊，技能照常 | 同上 | 綠 |
...
| 林海之王 | 52 | 被動「豐饒」：我方生物 ♥ 上限 +1 |
| 潮影雙生 | 39 | 天生技 低語（能量 4，每局 2 次）：對手隨機棄 1 張 |
...
相對變強，潮影雙生降到 39；照上面的效果改完後烈焰領主偏強、林海之王偏弱，各調到 51 與 52；稜光限 3 次後虹彩賢者偏弱，調到 47。英雄進化加的 HP 是 9–11。
...
| | 這個分支（攻擊 = 費用） | v0.8 |
|---|---|---|
| 新制先攻勝率（三種打法） | 52.9% – 54.0% | 51.6% – 53.0% |
| 基準英雄的預估時間（中位數） | 6.0 分（55 HP） | 6.7 分（35 HP） |
| 落在 5–10 分鐘 | **87%** | 78% |
| 英雄對其他英雄的平均勝率 | 45.0% – 53.1% | 46.6% – 54.8% |

| 英雄 HP（模擬英雄） | 35 | 45 | **55** | 65 | 75 |
|---|---|---|---|---|---|
| 預估時間（中位數） | 4.8 分 | 5.5 分 | **6.0 分** | 6.6 分 | 7.1 分 |
| 落在 5–10 分鐘 | 43% | 70% | **87%** | 93% | 91% |

| 英雄 | HP | 鏡像先攻勝率 | 平均攻擊次數 | 預估時間（中位數） | 對其他英雄 |
|---|---|---|---|---|---|
| 烈焰領主 | 51 | 52.6% | 11.6 | 6.8 分 | 53.1% |
| 無名劍士 | 53 | 55.5% | 23.2 | 5.5 分 | 52.9% |
| 林海之王 | 52 | 53.9% | 27.3 | 5.7 分 | 49.6% |
| 虹彩賢者 | 47 | 51.4% | 19.6 | 6.3 分 | 49.4% |
| 潮影雙生 | 39 | 54.6% | 22.3 | 7.1 分 | 45.0% |

1. **對局時間比 v0.8 穩**：55 HP 有 87% 落在 5–10 分鐘；65 HP 可以到 93%，但平均多半分鐘。
   免費的攻擊讓每回合都有事做，局面推進得比較平均。
2. **先攻比較有利**：52.9%–54.0%，比 v0.8 高 1–2 個百分點。場上的生物每回合都能免費攻擊，
   先下場的一方先搶到節奏，跟爐石一樣。試玩也有這個感覺的話，可以給後攻補償（例如爐石的幸運幣）。
3. **英雄差距不大**：45.0%–53.1%。潮影雙生的低語限 2 次之後稍微偏弱，試玩覺得弱的話可以加一點 HP。
4. 30 張的牌組讓牌庫抽完的對局變多一點（0.8%，v0.8 是 0.3%），還不是問題。
