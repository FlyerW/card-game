...
> 狀態：v0.7 規則草案｜規則引擎已實作（`packages/engine`，全部 168 個測試）｜網頁版可以跟電腦試玩、自己組牌（`packages/web`）｜最後更新：2026-09-26
...
- 讓自己的生物退場：不花能量，連同進化前的牌和道具送進棄牌區，空出格子（對手可以回應）
...
所以文件和程式永遠一致。目前有 5 名英雄、88 張卡，數值都照 v0.6 的數值基準與稀有度預算調過：

- **每個顏色 14–16 張、無色 8 張**（英雄進化卡另計）。單色英雄有 22–24 種卡可以用（再加上自己的英雄進化卡），組得出正式的 40 張牌組
- **9–12 費的高費卡**，每個顏色 2 張、無色 2 張：
  攻城巨像（9，無色）、深海巨妖（9，藍，進場對手全體麻痺）、腐朽之王（9，黑，進場對手全體中毒 2）、
  隕石術（9，紅，任意目標 18 加全體 2）、天使長（10，白，進場回復 8）、山嶺巨人（10，綠，進場能量上限 +1）、
...
以下是 v0.7 的卡池（88 張，含 9–12 費的高費卡）、英雄 35 HP 的結果，共 46000 局。
完整數據與限制見 [balance-results.md](balance-results.md)。

| 能量制度 | 先攻勝率（三種打法） | 判讀 |
|---|---|---|
| 舊制：從 1 開始每回合 +1、最高 10、後攻第一回合補 1 點 | 62.3% – 64.3% | 先攻明顯有利 |
| **新制：先攻 1、後攻 2，每回合 +2、最高 12** | **49.3% – 51.4%** | **看不出差距** |
| 新制但後攻不補償：雙方都從 1 開始 | 70.9% – 73.3% | 先攻明顯有利 |

| 英雄 HP（新制、均衡） | 25 | 30 | **35** | 40 | 50 |
|---|---|---|---|---|---|
| 先攻勝率 | 48.6% | 49.0% | **50.2%** | 50.2% | 50.3% |
| 平均回合數（雙方合計） | 16.1 | 17.6 | **18.9** | 20.1 | 22.3 |
| 預估時間（中位數） | 5.9 分 | 6.5 分 | **7.1 分** | 7.6 分 | 8.6 分 |
| 落在 5–10 分鐘的比例 | 64.3% | 73.0% | **77.0%** | 76.1% | 68.8% |

| 英雄 | 鏡像先攻勝率 | 預估時間 | 對其他英雄的平均勝率 |
|---|---|---|---|
| 烈焰領主（紅） | 52.5% | 5.3 分 | **75.8%** |
| 無名劍士（白） | 51.4% | 8.6 分 | 66.5% |
| 林海之王（綠） | 48.6% | 7.9 分 | 40.0% |
| 潮影雙生（藍黑） | 48.1% | 8.2 分 | 35.8% |
| 虹彩賢者（五色） | 49.1% | 5.5 分 | 31.8% |

**結論**

1. **新制是平衡的。** 後攻從 2 開始，剛好抵銷先攻搶先行動的優勢。沒有這 1 點，先攻勝率超過七成——
   能量每回合 +2，搶先一步的價值比舊制大得多。三種打法下都看不出後攻有利，補償沒有過頭。
2. **英雄 HP 不影響先後手平衡，只影響對局長度**：每多 5 HP，平均多 1–1.5 個回合。
   **35 HP 最符合一局 5–10 分鐘**：預估中位數 7.1 分鐘，77% 的對局落在 5–10 分鐘，是幾個 HP 裡最高的。
3. **英雄之間差距很大**：紅色對其他英雄平均贏 75.8%，對虹彩賢者贏到 89.5%；
   白色次之（66.5%）；綠色、潮影雙生、虹彩賢者都在四成以下。紅色的對局也最短（5.3 分），
   多數是在其他英雄的高費卡上場前就打完了。電腦只看一步、偏好直接打臉，可能高估紅色，
   但差距大到試玩時應該也感覺得到，見[待你決定](#待你決定)。
4. **電腦幾乎不存牌回應**：紅色的灼熱射線、綠色的巨化術在自己的回合就直接打出去，從來沒有留到回應時機；
   一局裡要想一下要不要回應的次數，白色平均 2.7 次，其他英雄不到 1 次。真人會存牌回應的話，對局會比預估長一點。
5. 高費卡的快速測試見[數值基準](#數值基準)的「高費生物」。

以前幾版的模擬（起手 5 張或 4 張、共用或各自的場地區、英雄 50 HP、調整前的範例卡）結論都相同：
新制平衡、HP 只影響長度。限制是電腦對手只看一步、比人弱，實際的先攻優勢可能比這裡量到的大一點。
...
2. **英雄強弱**：電腦對打裡紅色（烈焰領主）明顯最強，對其他英雄平均贏 75.8%；白色 66.5% 次之；
   綠色 40.0%、潮影雙生 35.8%、虹彩賢者 31.8%（見[平衡模擬](#平衡模擬)）。加了高費卡後差距還在，
   因為紅色的對局平均 13.7 回合就結束，別人的大卡常常來不及上場。
   電腦只看一步、偏好直接打臉，可能高估紅色；要削紅色、補強其他英雄，還是等你試玩的感覺再決定。
