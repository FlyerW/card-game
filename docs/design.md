...
| 狀態 | 效果 | 持續 | 顏色 |
|---|---|---|---|
| **中毒 N** | 擁有者的**回合結束**時失去 N HP，**HP 上限也少 N**（回復補不回來） | 被擊倒或進化 | 黑 |
| **灼燒 N** | 擁有者的**回合結束**時受到 N 傷害（在中毒之後） | 被擊倒或進化 | 紅 |
...
- **中毒**扣的是上限：記成「少掉的上限」，不是傷害，所以再生、回復都補不回來；進化解除中毒，但少掉的上限不會回來。
  中毒在回合結束才發作，所以中毒的生物這回合還能先行動。
...
- **對手能量上限 −X**：讓對手少能量。能量上限每回合 +2、最高 12，對手已經在 12 的話 −2 下回合就補回來了，
  所以前期效果大、後期要給大一點的數字才有感：

| 生物 | 顏色 | 費用 | 效果 |
|---|---|---|---|
| 汲能海葵 | 藍 N | 2 | 進場：對手能量上限 −1 |
| 噬能魔 | 黑 R | 4 | 技能 吞噬（3）：對手能量上限 −2，你的能量上限 +1 |
| 封能石像 | 無色 R | 5 | 休息技能 封能：對手能量上限 −1 |
| 虛空學者 | 藍 SR | 6 | 進場：對手能量上限 −3；技能 洞悉（3）：抽 1 張 |
| 深淵吞噬者 | 黑 UR | 8 | 進場：對手能量上限 −4；技能 吞能（4）：對手 −3、你 +1；深淵之觸（5）：只打生物 7 傷害 |
...
| 英雄 | HP | 效果 |
|---|---|---|
| 無名劍士 | 53 | 被動「劍士之道」：我方生物 ⚔ +1 |
| 烈焰領主 | 51 | 天生技 燃燼（能量 1）：任意目標 1 傷害 |
| 林海之王 | 50 | 被動「豐饒」：我方生物 ♥ 上限 +1 |
| 潮影雙生 | 42 | 天生技 低語（能量 4，每局 2 次）：對手隨機棄 1 張 |
| 虹彩賢者 | 47 | 天生技 稜光（能量 3，每局 3 次）：抽 1 張 |

效果照回饋定。**天生技可以有每局次數限制**：潮影雙生的棄牌不限次數的話，對手到後來都沒有手牌；
虹彩賢者的抽牌也一樣限次數。英雄進化換成新的天生技時重新算次數（潮影君主的「深淵低語」也是 2 次）。
規則引擎也支援「只在我方回合」或「只在對方回合」生效的被動，目前的英雄沒有用到。

HP 的起點是 55 − 3 ×（顏色數 − 1）− 效果強度，再照英雄對戰的模擬調：數字放大後藍黑的控場（HP 減半、消滅、麻痺）
相對變強，潮影雙生降到 39；照上面的效果改完後烈焰領主偏強、林海之王偏弱，各調到 51 與 52；稜光限 3 次後虹彩賢者偏弱，調到 47。
異常狀態改成五種後，藍黑少了詛咒、潮影雙生偏弱，調回 42；林海之王偏強，調到 50。英雄進化加的 HP 是 9–11。

### 模擬結果

用 `npm run sim` 跑了 46000 局（每組 2000 局、英雄對戰每組 1000 局，30 張牌組、最多進化一次、進化線 2/2），
完整數據在 [balance-results.md](balance-results.md)。時間的估法跟 v0.8 一樣，另外每次攻擊算 3 秒。
這一版是攻擊與技能各一次、休息技能、進化卡速攻、原本二階的卡削弱、燃燼 1 費、異常狀態改成五種之後的結果。

| | 這個分支（攻擊 = 費用） | v0.8 |
|---|---|---|
| 新制先攻勝率（三種打法） | 51.2% – 52.5% | 51.6% – 53.0% |
| 基準英雄的預估時間（中位數） | 6.4 分（55 HP） | 6.7 分（35 HP） |
| 落在 5–10 分鐘 | **89%** | 78% |
| 英雄對其他英雄的平均勝率 | 46.1% – 53.7% | 46.6% – 54.8% |

| 英雄 HP（模擬英雄） | 35 | 45 | **55** | 65 | 75 |
|---|---|---|---|---|---|
| 預估時間（中位數） | 5.0 分 | 5.7 分 | **6.4 分** | 7.0 分 | 7.6 分 |
| 落在 5–10 分鐘 | 48% | 76% | **89%** | 92% | 88% |

| 英雄 | HP | 鏡像先攻勝率 | 平均攻擊次數 | 預估時間（中位數） | 對其他英雄 |
|---|---|---|---|---|---|
| 烈焰領主 | 51 | 54.3% | 10.8 | 6.9 分 | 53.7% |
| 林海之王 | 50 | 53.3% | 24.5 | 6.0 分 | 52.1% |
| 虹彩賢者 | 47 | 52.0% | 19.5 | 6.3 分 | 50.4% |
| 潮影雙生 | 42 | 54.6% | 22.7 | 7.8 分 | 47.8% |
| 無名劍士 | 53 | 56.8% | 23.2 | 5.5 分 | 46.1% |

1. **對局時間穩**：55 HP 有 89% 落在 5–10 分鐘。攻擊與技能各一次之後，一回合做的事變多，但對局也結束得比較快，時間跟之前差不多。
2. **先攻比較有利，但不多**：51.2%–52.5%。場上的生物每回合都能免費攻擊，先下場的一方先搶到節奏，跟爐石一樣。
   鏡像對戰裡無名劍士的先攻最有利（56.8%），他的被動讓生物一下場就壓人。試玩也覺得先攻太有利的話，可以給後攻補償（例如爐石的幸運幣）。
3. **英雄差距**：46.1%–53.7%。無名劍士對烈焰領主只有 31.4%（燃燼 1 費，每回合都能點掉他的小生物），但對林海之王 60.8%；
   五個英雄彼此有剋制，總平均還在可以接受的範圍。無名劍士試玩也覺得偏弱的話，可以加 2–3 HP。
4. 牌庫抽完的對局 0.4%，不是問題。
...
- 天生技可以有**每局次數限制**（潮影雙生 2 次、虹彩賢者 3 次）；進化換成新的天生技時重新算
- 牌庫抽完（回合開始時抽不到牌）就落敗
- 沒有屬性弱點
...
- **五種**：
  - **中毒**（回合結束時失去 HP，HP 上限也跟著少）、**灼燒**（回合結束時受到傷害）：一直扣到被擊倒或進化
...
規則寫在 `packages/economy`，全部是純函式（輸入舊的玩家資料、輸出新的），有測試；
目前網頁把玩家資料存在瀏覽器裡，之後有帳號就把同一份程式搬到伺服器上。

**新玩家**：收藏是五個英雄各一副起始牌組用到的卡（同一張取最多的那副），所以每個英雄一開始就組得出合法的牌組；
...
**收集要多久**（目前 112 種卡，收滿是 219 張；模擬 200 個新玩家，兌換卷都拿去換缺的卡）

| | 起始 | 80% | 90% | 收滿 |
|---|---|---|---|---|
| 要開的包數（中位數） | 0（116 張、83 種） | 38 包 | 65 包 | 140 包 |
| 每天拿滿 150 金幣要幾天 | — | 約 25 天 | 約 43 天 | 約 93 天 |

- 起始收藏已經有 83 種：虹彩賢者是五色，他的起始牌組就涵蓋很多顏色。想讓卡包更有吸引力，可以改成只送一副起始牌組、其他英雄用任務解鎖
- 卡的稀有度分布是 N 23、R 61、SR 23、UR 5 種：N 種類少、機率高，很快就收滿，之後開到的 N 都變成 N 兌換卷，但 N 兌換卷沒東西可換。
  可以考慮讓 N 兌換卷也能湊著換高一階（例如 5 張 N 換 1 張 R 兌換卷），或讓兌換卷換金幣

**美術**：用 AI 畫，風格與提示詞範本見 [art.md](art.md)。卡面現在是純文字與色塊，之後每張卡加一張插圖（依卡牌 id 對應檔名）。
...
- 你中毒的生物失去 HP 與上限，接著灼燒的生物受到傷害，然後換對手的回合。沒花完的能量不保留。
...
**異常狀態**：中毒、灼燒 2 ≈ 2 能量（每回合都會扣，但生物倒下或進化就沒了；中毒補不回來，比灼燒稍強）；
...
6. 金幣、每日任務、卡包、兌換卷：規則在 `packages/economy`，試玩版先存在瀏覽器裡；接帳號時搬到伺服器
