...
> 狀態：v0.7 規則草案｜規則引擎已實作（`packages/engine`，全部 165 個測試）｜網頁版可以跟電腦試玩、自己組牌（`packages/web`）｜最後更新：2026-09-25
...
- **範例卡擴充到 76 張**：每個顏色 10 張、無色 6 張，單色英雄組得出正式的 40 張牌組；
...
所以文件和程式永遠一致。目前有 5 名英雄、76 張卡，數值都照 v0.6 的數值基準與稀有度預算調過：
