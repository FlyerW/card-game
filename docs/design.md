...
| 虛空學者 | 藍 SR | 6 | 虛空奔流（能量上限 −4）：對手每隻生物 3 傷害 |
| 深淵吞噬者 | 黑 UR | 8 | 深淵獻祭（能量上限 −4）：任意目標 7 傷害；深淵之觸（5）：只打生物 7 傷害 |
...
| 潮影雙生 | 藍黑 | 42 | 天生技輪流：潮之面（能量 2）抽 1 張，用完變成影之面（能量 3）對手隨機棄 1 張，再變回來 |
| 虹彩賢者 | 五色 | 47 | 天生技 稜光（能量 3）：我方每隻生物 +1/+1 |

- 天生技都**沒有次數限制**。引擎還支援每局次數（`uses`），目前沒有英雄用。
- 英雄進化（都是 SR、6 費左右、HP +9～11）：

| 英雄進化 | 由誰進化 | 費用 | HP | 進場 | 天生技換成 |
|---|---|---|---|---|---|
| 無名劍聖 | 無名劍士 | 6 | +9 | 我方生物 +2/+2 | 劍意（2）：我方生物 ⚔ +1 |
