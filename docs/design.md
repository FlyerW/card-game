...
| 深海先知 | 藍 | 42 | 天生技 預見（能量 3）：抽 1 張 |
| 冥府祭司 | 黑 | 54 | 天生技 蝕心（能量 4）：對手隨機棄 1 張 |
| 烈焰領主 | 紅 | 51 | 天生技 燃燼（能量 1）：任意目標 1 傷害 |
| 林海之王 | 綠 | 58 | 天生技 萌發（能量 3）：能量上限 +1 |
...
| 潮影雙生 | 藍黑 | 42 | 天生技 低語（能量 4）：對手隨機棄 1 張 |
| 虹彩賢者 | 五色 | 47 | 天生技 稜光（能量 3）：抽 1 張 |

- 天生技都**沒有次數限制**。引擎還支援每局次數（`uses`），目前沒有英雄用。
- 英雄進化：無名劍聖、烈焰君王、萬木之王（天生技換成「萬木生長」：能量上限 +1、抽 1 張）、潮影君主（深淵低語也不限次數）。
  深海先知、冥府祭司還沒有英雄進化卡。
- 規則引擎也支援「只在我方回合」或「只在對方回合」生效的被動，目前的英雄沒有用到。

HP 的起點是 55 − 3 ×（顏色數 − 1）− 效果強度，再照英雄對戰的模擬調（每組 200 局，對其他英雄的平均勝率 47.9%–52.6%）：
- 深海先知的抽牌不限次數，配上藍色的控場很強，要降到 42 才打平；林海之王的「萌發」前期很好用，但後期到 12 就沒用了，要 58。
- 翠林聖女只有被動，偏弱，58；冥府祭司的棄牌 4 費不便宜，54。
- 之前的調整：數字放大後藍黑的控場（HP 減半、消滅、麻痺）相對變強；中毒、灼燒改成施放者的回合結束就發作後，
  無名劍士的小生物更容易被磨掉，調到 59。英雄進化加的 HP 是 9–11。
...
這一版是英雄改成基礎五色加三個 UR 英雄、中毒灼燒在施放者的回合結束發作、多了「對手能量上限 −X」的生物之後的結果。

| | 這個分支（攻擊 = 費用） | v0.8 |
|---|---|---|
| 新制先攻勝率（三種打法） | 51.5% – 53.4% | 51.6% – 53.0% |
| 基準英雄的預估時間（中位數） | 6.3 分（55 HP） | 6.7 分（35 HP） |
| 落在 5–10 分鐘 | **88%** | 78% |
| 英雄對其他英雄的平均勝率 | 48.3% – 51.4%（8 個英雄） | 46.6% – 54.8%（5 個英雄） |

| 英雄 HP（模擬英雄） | 35 | 45 | **55** | 65 | 75 |
|---|---|---|---|---|---|
| 預估時間（中位數） | 4.8 分 | 5.6 分 | **6.3 分** | 6.9 分 | 7.5 分 |
| 落在 5–10 分鐘 | 43% | 72% | **88%** | 93% | 89% |

| 英雄 | HP | 鏡像先攻勝率 | 平均攻擊次數 | 預估時間（中位數） | 對其他英雄 |
|---|---|---|---|---|---|
| 冥府祭司 | 54 | 48.6% | 21.6 | 7.2 分 | 51.4% |
| 虹彩賢者（UR） | 47 | 51.4% | 19.8 | 6.1 分 | 50.9% |
| 林海之王 | 58 | 54.3% | 25.3 | 6.5 分 | 50.8% |
| 潮影雙生（UR） | 42 | 50.0% | 22.9 | 7.5 分 | 50.7% |
| 翠林聖女（UR） | 58 | 52.4% | 27.4 | 6.0 分 | 50.3% |
| 深海先知 | 42 | 52.8% | 20.8 | 6.5 分 | 49.2% |
| 烈焰領主 | 51 | 56.5% | 12.0 | 6.8 分 | 48.4% |
| 無名劍士 | 59 | 55.0% | 25.4 | 5.8 分 | 48.3% |

1. **英雄很平均**：八個英雄對其他英雄都在 48.3%–51.4%。UR 英雄沒有比基礎英雄強，抽到是多一種玩法，不是變強。
2. **對局時間穩**：55 HP 有 88% 落在 5–10 分鐘。黑色、藍黑的棄牌讓對局拖長一點（7.2–7.5 分）。
3. **先攻稍微有利**：51.5%–53.4%。鏡像對戰裡烈焰領主（56.5%）與無名劍士（55.0%）的先攻最有利，他們搶節奏。
   試玩也覺得先攻太有利的話，可以給後攻補償（例如爐石的幸運幣）。
4. 牌庫抽完的對局 0.5%，不是問題。
...
**收集要多久**（目前 117 種卡加 3 個 UR 英雄，收滿是 231 張；模擬 200 個新玩家，兌換卷都拿去換缺的卡）

| | 起始 | 80% | 90% | 收滿 |
|---|---|---|---|---|
| 要開的包數（中位數） | 0（122 張、83 種） | 40 包 | 69 包 | 208 包 |
| 每天拿滿 150 金幣要幾天 | — | 約 27 天 | 約 46 天 | 約 139 天 |

- 收滿要的包數比之前多很多（140 → 208），因為 UR 那一格現在有 9 種（6 張 UR 卡、3 個 UR 英雄），最後幾個 UR 很難抽到，多半要靠 UR 兌換卷。
- 起始收藏已經有 83 種：五個基礎英雄各一副起始牌組，涵蓋了每個顏色。想讓卡包更有吸引力，可以改成只送一副起始牌組、其他英雄用任務解鎖
- 卡的稀有度分布是 N 24、R 63、SR 24、UR 9 種：N 種類少、機率高，很快就收滿，之後開到的 N 都變成 N 兌換卷，但 N 兌換卷沒東西可換。
