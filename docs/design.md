...
- **沒花完的能量會留到對手的回合**，用來在對手的回合即時回應（見[提案](#提案即時回應怎麼運作)）。
  回應機制還沒實作，目前留下的能量只是保留著。
...
英雄進化與異常狀態都已經定案，移到規則草案。剩下即時回應還有細節要確認。

### 提案：即時回應怎麼運作

已定的方向：像魔法風雲會，用存下來的能量，在對手的回合即時回應。以下是建議的做法：

**什麼可以用來回應**

- 新的卡種 **瞬發法術**：可以在對手的回合施放。一般法術仍然只能在自己的回合用。
- 生物技能可以標【瞬發】，這種技能能在對手的回合發動。仍然算在這隻生物「每回合一個技能」裡——
  這裡的「每回合」是每一個回合，所以牠在對手的回合用了瞬發技能，不影響牠在自己回合的那一次。
- 付費一律用**存下來的能量**。

**什麼時候可以回應**

- 對手每次**宣告**一個動作（召喚、進化、發動技能、天生技、施放法術、放道具、放場地卡）之後、
  **效果結算之前**。
- 對手**宣告回合結束**時，還有最後一次機會把留下的能量用掉——下個回合開始時能量就會重置。
- 你自己的回合，對手也可以用他存下來的能量回應你。兩邊是對稱的。

**連鎖**

- 回應也可以被回應，形成連鎖，**後打出的先結算**。例如對手發動攻擊 → 你用瞬發法術讓牠 HP 減半 →
  對手再用瞬發法術回復——結算順序是回復、減半、攻擊。
- 連鎖中途有單位倒下或目標消失，後面針對它的效果就不發動（跟現在的規則一樣）。

**線上對戰的節奏**

即時回應最大的代價是每一步都可能要等對方決定。建議這樣處理：

- **沒有存能量的一方直接跳過**，不需要等。多數時候對手的能量已經花光，節奏不受影響。
- 有存能量時，給一段短時間（例如 8 秒）決定，時間到自動跳過；也可以設定「這個回合都不回應」。
- 要注意：如果「沒有瞬發牌」也自動跳過，對手就能從跳過的速度推測你手上沒有瞬發牌。
  所以**只要有存能量，不管手上有沒有瞬發牌，都一律等一下**，資訊才不會外洩。

**規則引擎的改動**

目前每個動作宣告後立刻結算。改成即時回應後，狀態裡要多一個「等待結算的連鎖」，
非當前回合的玩家也要能送出動作。這是引擎到目前為止最大的一次結構改動，建議等以上細節確定後再做。

---

## 待你決定

1. **即時回應**：瞬發法術與【瞬發】技能、回應時機、連鎖後發先至、節奏的處理方式，這樣可以嗎？
   （[提案](#提案即時回應怎麼運作)）
2. **異常狀態要先做進規則引擎嗎？** 規則已經定了，可以單獨先做；即時回應是比較大的結構改動，可以等細節確定後再做。
