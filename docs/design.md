...
異常狀態改成五種後，藍黑少了詛咒、潮影雙生偏弱，調回 42；林海之王偏強，調到 50。
中毒、灼燒改成施放者的回合結束就發作後，無名劍士的小生物更容易被磨掉，調到 59。英雄進化加的 HP 是 9–11。
...
| | 測試帳號 | Google 帳號 |
|---|---|---|
| 一開始 | 10000 金幣、全部的卡都收滿 | 100 金幣、五個英雄的起始牌組 |
...
7. 登入：測試帳號（存在瀏覽器）與 Google 帳號（伺服器驗證 ID token，資料存在伺服器）
