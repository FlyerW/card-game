...
- **休息技能**：標「休息」的技能**不花能量**，但這回合**還沒攻擊**才能用，用了這回合就不能攻擊。
  給挑釁、增益這類「這回合不打、換別的好處」的效果：縮殼、挑釁、守護、磨刀、年輪、深根、聖盾。
...
每隻生物有一個種族，大多數有那個種族的特色，**特色有強度**（同袍 1、同袍 2……），寫在卡上。
沉默時種族特色跟卡上其他效果一樣失效；進化線上的卡同一個種族。

| 種族 | 特色 | 預設 | 不一樣的卡 |
|---|---|---|---|
| 人類 | **同袍 N**：我方場上有其他人類時 ⚔ +N | 1 | 2：流浪傭兵、聖騎士、血祭術士；沒有：聖泉修女、虛空學者 |
| 野獸 | **猛撲**：召喚當回合就能攻擊生物（不能打英雄） | 有 | 沒有：岩殼龜、小水母 |
| 亡靈 | **不死 N**：第一次被打倒時留下 N♥（消滅不算打倒） | 1 | 2：骷髏騎士、死亡騎士、腐朽之王 |
| 元素 | **元素之力 N**：技能、進場與遺言的傷害 +N | 1 | 沒有：湧泉精靈 |
| 植物 | **扎根 N**：你的回合開始時回復 N♥ | 1 | 2：古樹之根、藤蔓巨像 |
| 龍 | **龍鱗**：不會中異常狀態（同一張卡的其他效果照樣結算） | 有 | — |
| 機械 | **堅固 N**：受到的傷害 −N | 1 | 2：攻城巨像 |
| 天使 | **光輝 N**：召喚時你的英雄回復 N♥ | 3 | 2：聖光使者；4：光之巨神 |

- 原本每張卡的特色都一樣大，改成寫在卡上的強度：大卡可以強一點，也可以整張拿掉（例如烏龜不會撲人）。
  「其他人類」只看種族，沒有同袍的人類一樣會讓別的人類 +N。
- 構造體改名機械。
- 特色都很小，數值沒有另外扣；照模擬調英雄 HP。
- **亡靈的「不死」會不會太強？**用 8400 局英雄對戰看牌組裡亡靈的張數：0 張 50.2%、1–3 張 50.3%、4–6 張 48.7%、7 張以上 51.1%，
  帶越多沒有越強；每張亡靈卡 49.4%–53.9%（深淵吞噬者最高）。其他種族也一樣，牌組裡某種族的卡多寡對勝率影響不大。
  不過電腦不會刻意利用不死（例如明知會留 1♥ 就先換掉對手的大生物），真人玩起來可能比較強，試玩時留意。
...
- **衍生物（token）**：只能由效果召喚、不能放進牌組，放在自己最左邊的空格，召喚當回合一樣不能攻擊；
  被擊倒或退場就消失，不進棄牌區。士兵（白 ⚔2 ♥2）：集結號令（4 費召喚 2 隻）、騎士隊長（5 費，進場帶 1 隻，技能再叫 1 隻）、殉道騎士的遺言；
  骷髏（黑 ⚔1 ♥1 亡靈）：屍骨領主的遺言；烈陽騎兵（白紅 ⚔1 ♥1 人類、速攻）：烈陽統帥的天生技。
  說明文字會帶上衍生物的數值，例如「召喚 1 隻烈陽騎兵（1/1、速攻）」。
- **遺言**：生物死掉時發動，見[遺言](#遺言)。
- **持續效果**：生物在場上時，每當條件成立就自動發動，不選目標、不花能量，沉默中不發動。算一個技能的份量（R、SR 有持續效果就沒有技能）。
  時機有三種：**回合開始**（抽牌、再生之後）、**回合結束**（中毒、灼燒之前）、**每當回復**（每當我方英雄回復 ♥）。
  「每當回復」發動的效果如果又回復英雄，不會再觸發一次，免得無限循環。

| 生物 | 顏色 | 費用 | 數值 | 持續效果 |
|---|---|---|---|---|
| 聖光守護者 | 白 R 天使 | 4 | 2/5 | **每當回復** 聖光共鳴：自身 +1/+1（召喚時的光輝也算） |
| 星象學者 | 藍 R 人類 | 4 | 3/4 | **回合結束** 觀星：抽 1 張 |
| 腐化祭司 | 黑 R 人類 | 4 | 3/4 | **回合結束** 腐化：對手每隻生物中毒 1 |
| 烈焰之心 | 紅 SR 元素 | 6 | 5/6 | **回合結束** 餘燼風暴：對手每隻生物 1 傷害（元素之力再 +1） |
| 生命古樹 | 綠 R 植物 | 5 | 2/7 | **回合開始** 生命之泉：我方英雄與每隻生物回復 2 |

- **技能召喚衍生物**：死靈術士（黑 R 4 費 3/4，喚骨（3）召喚 1 隻骷髏）、森林守護者（綠 R 4 費 3/5，播種（2）召喚 1 隻小樹精 1/2 植物）、
  火焰召喚師（紅 R 4 費 3/3，召火（3）召喚 1 隻火靈 2/1 元素**速攻**），加上原本的騎士隊長（白，號召召喚士兵）。
- **卡面文字用關鍵字**：常見的效果寫成粗體的關鍵字（**中毒 2**、**遺言**、**速攻**、**同袍 2**），點卡片看說明時，
  下面用小字列出用到的關鍵字是什麼意思。卡面短，新手也查得到。
...
| 英雄 | 顏色 | HP | 效果 |
|---|---|---|---|
| 無名劍士 | 白 | 44 | 被動「劍士之道」：我方生物 ⚔ +1，在自己的回合再 +1（攻擊 +2、反擊 +1）；我方生物攻擊時，正前方與斜對角都被擋住也打得到英雄 |
| 深海先知 | 藍 | 44 | 天生技 預見（能量 3）：抽 1 張 |
| 冥府祭司 | 黑 | 37 | 天生技 蝕心（能量 5）：對手隨機棄 1 張 |
| 烈焰領主 | 紅 | 45 | 天生技 燃燼（能量 2）：任意目標 2 傷害 |
| 林海之王 | 綠 | 45 | 天生技 萌發（能量 3）：能量上限 +1，我方英雄與每隻生物回復 2 |
...
| 英雄 | 顏色 | HP | 效果 |
|---|---|---|---|
| 翠林聖女 | 白綠 | 39 | 被動「豐饒」：我方生物 ♥ 上限 +2 |
| 潮影雙生 | 藍黑 | 35 | 天生技輪流：潮之面（能量 4）抽 1 張，用完變成影之面（能量 4）對手隨機棄 1 張，再變回來 |
| 烈陽統帥 | 白紅 | 38 | 天生技 衝鋒號（能量 2）：召喚 1 隻烈陽騎兵（1/1、人類、速攻，召喚當回合就能攻擊） |
| 虹彩賢者 | 五色 | 40 | 天生技 稜光（能量 3）：我方每隻生物 +1/+1 |
...
| 冥府之主 | 冥府祭司 | 6 | +10 | 對手每隻生物中毒 2 | 奪心（4）：對手隨機棄 1 張 |
| 烈焰君王 | 烈焰領主 | 6 | +9 | 對手每隻生物 2 傷害 | 煉獄（2）：任意目標 3 傷害 |
| 萬木之王 | 林海之王 | 5 | +11 | 能量上限 +1 | 萬木生長（3）：抽 1 張、我方英雄與每隻生物回復 3 |
| 潮影君主 | 潮影雙生 | 6 | +11 | 對手隨機棄 2 張 | 深淵低語（5）：對手隨機棄 1 張、抽 1 張 |

  翠林聖女、烈陽統帥、虹彩賢者還沒有英雄進化卡（多色英雄的技能之後一起設計）。
- 規則引擎也支援「只在我方回合」或「只在對方回合」生效的被動，目前的英雄沒有用到。

**英雄 HP 在 35–45 之間**：HP 低一點快攻才打得死。英雄的 HP 沒有上限，回復可以補到比起始 HP 還高。
HP 與技能用**困難電腦**的英雄對戰模擬調（它會模擬對手的回應，比普通電腦準），
目標是**每個英雄對其他英雄的平均勝率都在 45%–55%**，結果見[模擬結果](#模擬結果)。
HP 從 36–66 降到 35–45 時，只調 HP 調不平，所以也改了技能：

- **無名劍士**：HP 已經到 45，改成在自己的回合攻擊再 +1（44% → 51%）。
- **烈焰領主**：燃燼 1 費 1 傷害偏弱（43%）；2 費 3 傷害、3 費 4 傷害都太強（55%、59%，一發能解的生物變多），定在 2 費 2 傷害（48%）。
- **林海之王**：萌發多了全體回復 2，HP 45。萌發試過 2 費（52%），但那樣比能量結晶（2 費上限 +1）、
  森林之息（2 費全體回復 3）都划算，林海之王的牌組就不會帶這兩張，所以留在 3 費。
  進化後的萬木生長改成抽 1 張、全體回復 3（接近能量上限 12 時，加上限沒有用）。
- **翠林聖女**：豐饒 ♥ +1 → +2（HP 低了之後生物要更耐打）。
- **抽牌、棄牌調貴**：潮影雙生的潮之面 3 → 4 費，冥府祭司的蝕心 4 → 5 費；進化後的奪心 3 → 4、深淵低語 3 → 5。
  HP 變低、對局變短之後，累積手牌優勢的英雄反而更強（困難電腦下潮影雙生原本 62%）。

以前（HP 36–66、普通電腦）調過的：
...
英雄 HP 改成 35–45 之後用**困難電腦**重量（2800 局英雄對戰）：**120 張裡 117 張在範圍內**。超出的三張都在邊緣：
天使長 59.0%（只有 332 局，誤差約 ±5）、古樹熊神 56.9%、反制電流 44.7%，先不動，之後局數多了再看。
普通電腦量出來白卡普遍 60% 以上、藍黑卡 40% 出頭，那是普通電腦下英雄強弱不一，不是卡的問題。

加了遺言、持續效果、技能召喚衍生物的 24 張新卡與烈陽統帥之後（困難電腦，3600 局英雄對戰）：

- **新卡都在範圍內**（47.8%–53.2%）。
- 白、黑、紅多了好用的新卡，**藍色整體掉到 45% 左右**（深海先知 44%）。補強藍色：深海先知 HP 41 → 44、星象學者 5 → 4 費、
  卷軸學徒 2/2 → 2/3、知識之泉 4 → 2 費；虹彩賢者 HP 37 → 40。之後藍色卡平均 48.6%，136 張裡 128 張在範圍內。
- 剩下超出的做最後一輪小調整：影噬魔的蝕魂 4 → 5 費（57%）、深海巨妖 HP 11 → 10（57%）、瘟疫的全體中毒 2 → 1（56%）、
  腐朽之王的不死 3 → 2（55%）、藤蔓巨像 HP 9 → 10（45%）。
- 調完再量一次：**136 張裡 131 張在範圍內**，英雄 45.4%–54.0%。超出的五張都離邊界不到一個誤差：
  影噬魔 56.0%、深海巨妖 55.6%、九尾天狐 55.2%、冰錐 44.6%、大地泰坦 44.2%（只有 319 局）。

### 模擬結果

**現在（英雄 HP 35–45、困難電腦）**：英雄兩兩對戰，每組 100 局
（`BOT=hard` 讓模擬用困難電腦；它一步要想約 30 毫秒，比普通電腦慢 20 多倍，所以局數少）。
一個英雄對其他英雄的平均是 800 局，誤差大約 ±2 個百分點。

| 英雄 | HP | 對其他英雄（加新卡後） | 加新卡前 | 鏡像先攻勝率 | 預估時間（中位數） |
|---|---|---|---|---|---|
| 翠林聖女（UR） | 39 | 53.3% | 50.7% | 55% | 6.5 分 |
| 冥府祭司 | 37 | 52.9% | 50.9% | 48% | 6.6 分 |
| 無名劍士 | 44 | 52.8% | 50.6% | 49% | 5.2 分 |
| 烈焰領主 | 45 | 51.0% | 47.6% | 50% | 6.7 分 |
| 潮影雙生（UR） | 35 | 50.1% | 51.1% | 56% | 8.1 分 |
| 烈陽統帥（UR） | 38 | 50.0% | 47.9% | 56% | 7.1 分 |
| 深海先知 | 44 | 47.8% | 49.1% | 53% | 9.4 分 |
| 虹彩賢者（UR） | 40 | 46.9% | 48.4% | 40% | 6.5 分 |
| 林海之王 | 45 | 45.4% | 48.8% | 51% | 8.0 分 |

「加新卡後」是 24 張新卡加進來、藍色補強之後量的（HP 是現在的值）；鏡像先攻勝率與時間是加新卡前量的。

1. **九個英雄都在 45.4%–53.3%**。林海之王最低，貼近下限，之後再看。
2. **先後攻**：拿掉後攻多的 1 點能量之後，英雄鏡像的先攻勝率平均約 49%。每組只有 100 局，單一英雄的 40%–56% 大多是誤差。
3. **對局變短**：無名劍士的對局中位數 5.2 分，快攻確實打得比較快；深海先知最長（9.4 分）。

以下是 HP 還在 36–66、用普通電腦時的結果，留著比較：

用 `npm run sim` 跑了 70000 局（每組 2000 局、英雄對戰每組 1000 局，30 張牌組、最多進化一次、進化線 2/2），
完整數據在 [balance-results.md](balance-results.md)（這次調整之前產生的）。時間的估法跟 v0.8 一樣，另外每次攻擊算 3 秒。
...
- **後攻沒有另外補償**，只有第一回合的能量上限多 1（2 比 1）。原本後攻第一回合再多補 1 點能量，試玩後拿掉；
  用困難電腦模擬，拿掉之後八個英雄鏡像的先攻勝率平均 48.9%（各 40%–56%，每組 100 局）。試過後攻起手多 1 張，補過頭（先攻只剩 41%–50%）。
...
- **名字＋密碼帳號**：不用 Google，名字（不能重複）加密碼就能開，登出或換電腦都能登回來。密碼用 scrypt 加鹽雜湊存，不存明文。
  一開始做成「訪客帳號」（只靠瀏覽器記得登入），登出就再也回不去；改成名字＋密碼時，舊的訪客帳號第一次用原本的名字登入就設定密碼，
  同名的舊帳號全部合併成一個（抽到的卡加在一起，不超過上限；兌換卷與金幣相加）。
...
4. 先攻玩家開始第 1 回合。後攻第一回合的能量上限是 2（先攻是 1）。
...
- **吸血**（關鍵字）：這隻生物的技能與進場效果造成傷害時，**自己的英雄回復等量的 HP**。
  打生物時算減傷後實際造成的傷害；範圍傷害把打到的每一隻加起來。
- **再生 N**：擁有者的回合開始時，這隻生物回復 N HP，不超過上限。可以寫在卡上，
  也可以來自英雄被動或場地卡，兩者相加。
- 再生在自己的回合開始時結算，補得回中毒與灼燒扣的血。
- 另外有**全體回復**：我方英雄與每隻生物各回復 N（森林之息、山嶺巨人的進場）。
- **英雄的 HP 沒有上限**：回復、吸血、光輝都可以把英雄補到比起始 HP 還高（例如 40 HP 的英雄補到 48）。
  生物的 HP 還是有上限。

#### 遺言

- 生物**死掉時**發動：HP 歸零被移出場，或被消滅。亡靈的「不死」第一次倒下留 1 HP，不算死，要第二次才發動。
- **不選目標**，也不另外花能量；價值約同樣效果的法術打七折（要先死才拿得到），從本體數值扣。
- **死的時候在沉默中就不發動**（沉默讓卡上的效果失效）。場上的生物有遺言時會標「遺言」。
- 召喚衍生物的遺言，第一隻放回死掉那隻原本的格子，其餘照樣放最左邊的空格。
- 同時死很多隻（例如互打、範圍傷害）：回合玩家的先，同一邊由左到右；遺言打死的生物會再發動牠們的遺言。
- 先檢查英雄：英雄已經倒下、對局結束了，遺言就不發動（遺言的回血救不回已經歸零的英雄）。

| 生物 | 顏色 | 費用 | 數值 | 遺言 |
|---|---|---|---|---|
| 祝福朝聖者 | 白 N | 2 | 2/2 | 最後的祈禱：我方英雄回復 4 |
| 殉道騎士 | 白 N | 3 | 2/3 | 傳承：召喚 1 隻士兵（2/2） |
| 卷軸學徒 | 藍 N | 2 | 2/3 | 遺稿：抽 1 張 |
| 冰晶守衛 | 藍 SR | 5 | 4/6 | 碎冰：對手每隻生物麻痺（另有技能 冰刺（2）任意目標 3） |
| 瘟疫行屍 | 黑 R | 4 | 3/4 | 屍毒：對手每隻生物中毒 2（亡靈，要死兩次；另有技能 腐爪（2）目標生物中毒 2） |
| 屍骨領主 | 黑 SR | 6 | 5/7 | 亡者召集：召喚 2 隻骷髏（1/1 亡靈）（另有技能 骨刺（3）任意目標 4） |
| 自爆工兵 | 紅 N | 3 | 2/2 | 引爆：對手每隻生物 2 傷害 |
| 古樹之根 | 綠 R | 4 | 3/5 | 歸根：我方英雄與每隻生物回復 4（另有技能 藤鞭（2）只打生物 4） |
...
- **召喚當回合就能進化**（進化卡都有速攻），進化完馬上能攻擊或發動技能；這回合已經攻擊或發動過的話，進化不會多給一次。
- **最多進化一次**：基礎 → 進化。
- 進化卡不能直接召喚。
...
**誰能打**：伺服器上的帳號——名字＋密碼帳號或 Google 帳號。測試帳號存在瀏覽器裡，不能打排位。
...
- 贏 +1 星，鑽石以下連勝 2 場起每場再多 +1；輸 −1 星，星星 0 再輸掉一段。**銅牌、銀牌不會掉段**，大師整季不會掉。
...
7. 登入：測試帳號（存在瀏覽器）、名字＋密碼帳號與 Google 帳號（存在伺服器）
8. 排位賽、新手教學、困難的電腦對手
