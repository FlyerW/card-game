...
// 用法：npm run sim -- [--games 4000] [--workers 16]
...
function report(summaries: Summary[], seconds: number): string {
  ...
  const row = (s: Summary) =>
    ...
    `${pct(s.ci[0])} – ${pct(s.ci[1])} | ${verdict(s)} | ${s.meanTurns.toFixed(1)} | ${s.p10Turns}–${s.p90Turns} | ${s.meanWinnerHp.toFixed(1)} |`;
  const header =
    '| 能量制度 | 打法 | 英雄 HP | 先攻勝率 | 95% 信賴區間 | 判讀 | 平均回合數 | 回合數 10–90% | 勝方剩餘 HP |\n' +
    '|---|---|---|---|---|---|---|---|---|';
  const energy = summaries.filter((s) => s.experiment.heroHp === 50);
  const hp = summaries.filter((s) => s.experiment.id.startsWith('new/balanced/')).sort((x, y) => x.experiment.heroHp - y.experiment.heroHp);
